...
class MolecularConstituent:
    ...
    def constructLookupFunction(self):
        file_path = molecularLookupPath + self.moleculeName + '.dat'
        print(f"Attempting to open file: {file_path}")  # Debug statement
        ...
        # Read the temperature and pressure grids
        try:
            temperature_grid = np.array([float(x) for x in lines[0].split()])
            pressure_grid = np.array([float(x) for x in lines[1].split()]) * 10.0  # Convert to cgs-units
        except Exception as e:
            print(f"Error reading temperature and pressure grids: {e}")
            raise

        print("Temperature grid:", temperature_grid)
        print("Pressure grid:", pressure_grid)

        # Parse the wavelengths and opacities
        wavelengths = []
        opacities = []
        ...
        while index < len(lines):
            ...
            try:
                wavelength = float(lines[index].strip())
                wavelengths.append(wavelength)
                index += 1
            except ValueError as e:
                print(f"Error reading wavelength at line {index}: {e}")
                raise
            ...
            while index < len(lines) and lines[index].strip():
                try:
                    values = [float(x) for x in lines[index].split()]
                    if len(values) < 2:
                        break  # Move to the next wavelength block if the line doesn't have enough values
                    pressure = values[0]
                    opacity_values = values[1:]
                    if len(opacity_values) != len(temperature_grid):
                        print(f"Line {index}: Expected {len(temperature_grid)} opacity values, but got {len(opacity_values)}")
                        raise ValueError(f"Number of opacity values does not match number of temperatures at line {index}")
                    opacity_block.append(opacity_values)
                    index += 1
                except ValueError as e:
                    print(f"Error reading opacity values at line {index}: {e}")
                    raise

            if len(opacity_block) != len(pressure_grid):
                print(f"Wavelength block {wavelength} has {len(opacity_block)} pressure blocks, expected {len(pressure_grid)}")
                raise ValueError(f"Number of pressure blocks does not match number of pressures for wavelength {wavelength}")

            opacities.append(np.array(opacity_block))

        wavelengths = np.array(wavelengths)
        opacities = np.array(opacities)

        print("Wavelengths:", wavelengths)
        print("Opacities shape before reshape:", opacities.shape)

        # Normalize opacities by the molecular mass and convert to cm²
        species = const.AvailableSpecies().findSpecies(self.moleculeName)
        if species is None:
            print(f"Species {self.moleculeName} was not found. Using default mass of 64.016 grams.")
            molecular_mass = 64.016 * 1.66e-24  # Convert to grams
        else:
            molecular_mass = species.mass
        
        opacities *= molecular_mass  # Convert from cm²/gram to cm²

        # Reshape opacities to (pressure, temperature, wavelength)
        opacities = np.transpose(opacities, (1, 2, 0))

        print("Opacities shape after reshape:", opacities.shape)

        # Prepare the interpolation function
        lookupFunction = RegularGridInterpolator(
            (pressure_grid, temperature_grid, wavelengths),
            np.log10(opacities + self.lookupOffset),
            bounds_error=False,
    ...
    def addLookupFunctionToConstituent(self):
        print(f"Adding lookup function for {self.moleculeName}")  # Debug statement
        self.lookupFunction = self.constructLookupFunction()

    def getSigmaAbs(self, P, T, wavelength):  # P is an N_x array, T is a scalar, and wavelength is an (N_x, N_lambda) 2D array
