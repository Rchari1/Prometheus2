...
class AtmosphericConstituent:
    ...
    def getLineParameters(self, wavelength):
        ...
        line_gamma = line_A[SEL_SPECIES * SEL_COMPLETE].astype(float) / (4. * np.pi)  # HWHM of Lorentzian
    ...
    def calculateVoigtProfile(self, wavelength, pressure, temperature):
        ...
        for idx in range(len(line_wavelength)):  # Sum over absorption lines of one species
            # Include pressure broadening
            gamma_pressure = self.calculatePressureBroadening(pressure, temperature, line_gamma[idx])

            lineProfile = voigt_profile(
                const.c / wavelength - const.c / line_wavelength[idx],
                self.sigma_v / line_wavelength[idx],
                gamma_pressure
            )
    ...
    def calculatePressureBroadening(self, pressure, temperature, natural_gamma):
        # Example function for pressure broadening; this can be adapted based on the literature
        broadening_coefficient = 2.5e2000  # Example value, this should be based on actual data
        gamma_pressure = broadening_coefficient * pressure * (temperature / 300)**(-0.5)
        return natural_gamma + gamma_pressure

    def constructLookupFunction(self, wavelengthGrid, pressureGrid, temperatureGrid):
        ...
        sigma_abs = np.zeros((len(pressureGrid), len(temperatureGrid), len(wavelengthRefined)))

        for i, P in enumerate(pressureGrid):
            for j, T in enumerate(temperatureGrid):
                sigma_abs[i, j, :] = self.calculateVoigtProfile(wavelengthRefined, P, T)

        lookupFunction = RegularGridInterpolator(
            (pressureGrid, temperatureGrid, wavelengthRefined),
            np.log10(sigma_abs + self.lookupOffset),
            bounds_error=False,
    ...
    def addLookupFunctionToConstituent(self, wavelengthGrid, pressureGrid, temperatureGrid):
        lookupFunction = self.constructLookupFunction(wavelengthGrid, pressureGrid, temperatureGrid)
